...
describe("Xuper SDK Contract：", () => {
    ...
    test("create an new contact account", async () => {
        ...
        const execStruct = await xsdk.createContractAccount(
            parseInt(
                "1234567890" +
                    ~~(Math.random() * (999999 - 100000) + 100000).toString()
            )
        );
    ...
    test("deploy native contract", async () => {
        ...
        if (isBrowser) {
            ...
            const native = Uint8Array.from(
                fs.readFileSync(`${__dirname}/native/counter-native`)
            );
        ...
        try {
            ...
            } = await xsdk.deployNativeContract(
                contractAccount,
                contractName,
                codeBuf.join(""),
                "go",
    ...
    test("upgrade native contract", async () => {
        ...
        if (isBrowser) {
            ...
            const fs = require("fs");
            const native = Uint8Array.from(
                fs.readFileSync(`${__dirname}/native/counter-native-v2`)
            );
            native.forEach(n => codeBuf.push(String.fromCharCode(n)));
        }
    ...
    test("invoke native contract", async () => {
        ...
        try {
            const {
                transaction,
                preExecutionTransaction
            } = await xsdk.invokeContarct(contractName, "increase", "native", {
                key: "a"
            });
    ...
    test("deploy new wasm contract should return transaction info and result of post", async () => {
        ...
        if (isBrowser) {
            ...
            const wasm = Uint8Array.from(
                fs.readFileSync(`${__dirname}/wasm/counter.wasm`)
            );
            wasm.forEach(n => codeBuf.push(String.fromCharCode(n)));
        }

        const contractName = `counter${~~(Math.random() * 10 ** 3 - 10 ** 3) +
            10 ** 3}`;
    ...
    test("deploy a solidity contract", async () => {
        ...
        const bin =
            "";
        const abi =
            '[{"inputs":[{"internalType":"uint256","name":"num","type":"uint256"}],"payable":false,"stateMutability":"nonpayable","type":"constructor"},{"constant":false,"inputs":[],"name":"retrieve","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"internalType":"uint256","name":"num","type":"uint256"}],"name":"store","outputs":[],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"internalType":"uint256","name":"num","type":"uint256"}],"name":"storepay","outputs":[],"payable":true,"stateMutability":"payable","type":"function"}]';
        ...
        const {
            preExecutionTransaction,
            transaction
        } = await xsdk.deploySolidityContract(
            contractAccount,
            contractName,
            bin,
            abi,
    ...
    test("invoke Solidity contract", async () => {
        ...
        const {
            transaction,
            preExecutionTransaction
        } = await xsdk.invokeSolidityContarct(solidityName, "store", "evm", {
            num: "2"
        });

    ...
    func: {
        makeTransaction: async function(defaultArgs: any, account: AccountModel, ti: TransactionModel, authRequires: AuthModel[], preExecWithUtxosObj: any) {
            ...
            const checkTx = this.generateTransaction(
                account, {utxoOutput}, [], {amount: endorseFee, fee: 0, to: endorseServiceFeeAddr}
            );
        ...
        transfer: function(defaultArgs: any, chain: string) {
            const {fee: endorseFee, server, endorseServiceCheckAddr} = defaultArgs;
            return {[endorseServiceCheckAddr] : {
                fee: endorseFee,
